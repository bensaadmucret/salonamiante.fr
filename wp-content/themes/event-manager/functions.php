...
function child_theme_setup() {
	...
	include_once( CHILD_DIR . '/event-framework/lib/api/functions.php');
	...
	add_theme_support( 'genesis-custom-header', array( 'width' => 960, 'height' => 270, 'textcolor' => '333', 'admin_header_callback' => 'sc_admin_style', 'header_callback' => 'sc_custom_header_style' ) );	
...
function cmp($a, $b) 
{
    ...
    return ( strcasecmp($a_title, $b_title ) > 0 ) ? 1 : -1;
}
...
add_action ('init', 'itga_region_post_type');
function itga_region_post_type()
{
	// set up labels
    $labels = array(
        'name' => 'Région',
        'singular_name' => 'Nouvelles données',
        'add_new' => 'Ajouté une nouvelle donnée',
        'add_new_item' => 'Ajouté une nouvelle donnée',
        'edit_item' => 'Editer',
        'new_item' => 'Nouveau',
        'all_items' => 'Toutes les données',
        'view_item' => 'Voir',
        'search_items' => 'Recherche',
        'not_found' =>  'Pas de donnée trouvée',
        'not_found_in_trash' => 'Pas de données trouvé found in Trash',
        'parent_item_colon' => '',
        'menu_name' => 'Région',
    );
    register_post_type(
...
add_shortcode( 'region', 'rmcc_post_listing_parameters_shortcode' );
    ...
    while ( $query->have_posts() ) : $query->the_post(); ?>
        <div class="clothes-listing ">
            <div class="clothes-listing ">
              ...
              <p style="line-height: 1.5;">  <?= the_content(); ?> </p>
            </div>
        </div>
